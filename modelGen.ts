...
function generateClass (className: string, fields: any) {
  ...
  const templateData = fs.readFileSync("./modelGen.class.mustache").toString()
  return mustache.render(templateData, baseData)
}

function generateParser (className: string, fields: any) {
  const baseData = generateBaseData(className, fields)
  const templateData = fs.readFileSync("./modelGen.parser.mustache").toString()
  return mustache.render(templateData, baseData)
}

const targetDir = "./src/models"
const candidateFiles = fs.readdirSync("./src/models")
